...
def synthetic_input(params, global_step=None, eval=False):
    batch_size = params['eval_batch_size' if eval else 'train_batch_size']
    n_ctx = params['n_ctx']
    sample = [list(range(n_ctx))]
    tensor = tf.constant(sample, dtype=tf.int64)
    labels = tf.identity(tensor)
    dataset = tf.data.Dataset.from_tensors((tensor, labels))
