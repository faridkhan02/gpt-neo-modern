...
def detect_hand_gesture():
    ...
    while True:
        ...
        if contours:
            ...
            # Ignore tiny/noisy contours that aren't a real hand
            if cv2.contourArea(contour) > 3000 and len(contour) >= 5:
                ...
                hull_indices = np.sort(hull_indices, axis=0)  # must be ascending order
                defects = cv2.convexityDefects(contour, hull_indices)

                # Count the number of fingers extended (based on convexity defects)
                if defects is not None and defects.ndim == 3:
                    finger_count = 0
